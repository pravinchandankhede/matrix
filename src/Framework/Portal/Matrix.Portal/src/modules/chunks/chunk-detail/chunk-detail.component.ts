...
export class ChunkDetailComponent extends BaseDetailComponent<Chunk> implements AfterViewInit {
  ...
  createNewItem(): Chunk {
    const defaultStrategy: ChunkStrategy = {
      chunkStrategyUId: this.generateId(),
      name: 'Default Fixed Size',
      description: 'Fixed size chunking strategy',
      parameters: JSON.stringify({ chunkSize: 1000, overlap: 100 }),
      type: 'FixedSize',
      createdBy: 'System',
  ...
  updateItemFromForm(): void {
    ...
    if (this.item.chunkStrategy) {
      this.item.chunkStrategy.type = formValue.strategyType || 'FixedSize';
      this.item.chunkStrategy.parameters = JSON.stringify({ 
        chunkSize: formValue.chunkSize || 1000, 
        overlap: formValue.overlap || 100 
      });
      this.item.chunkStrategy.modifiedDate = new Date();
    }
  ...
  private populateForm(): void {
    if (this.item) {
      let parsedParams = { chunkSize: 1000, overlap: 100 };
      if (this.item.chunkStrategy?.parameters) {
        try {
          parsedParams = JSON.parse(this.item.chunkStrategy.parameters);
        } catch (e) {
          console.warn('Failed to parse chunk strategy parameters');
        }
      }
      ...
      this.chunkForm.patchValue({
        ...
        strategyType: this.item.chunkStrategy?.type || 'FixedSize',
        chunkSize: parsedParams.chunkSize || 1000,
        overlap: parsedParams.overlap || 100
      });
    }
  }
